...
### Pipeline Flow
```
MUTAG Dataset → k-GNN Training → GIN-Graph Generation → Explanation Evaluation → Visualization
```
...
- `DataConfig`: Dataset settings (MUTAG, 7 atom types, batch size)
...
### Output Directories
- `./checkpoints/`: Saved k-GNN models (`kgnn_*.pt`)
