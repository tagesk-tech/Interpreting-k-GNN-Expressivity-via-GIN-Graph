...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Two-layer MLP} with LeakyReLU activation: $\mathcal{D}(G) = \mathbf{w}^\top \text{LeakyReLU}_{0.2}(\mathbf{W}\,\mathbf{h}_G) \in \mathbb{R}$, where $\text{LeakyReLU}_\alpha(x) = \max(\alpha x,\, x)$ with negative slope $\alpha = 0.2$. The nonzero slope on the negative side keeps gradients flowing through units with negative pre-activations, unlike ReLU whose zero-gradient half-plane can produce dead units during adversarial training; LeakyReLU is the standard choice for GAN discriminators for this reason.
\end{enumerate}
