...
The dense wrapper re-implements the pretrained classifier's forward pass using dense matrix operations in place of sparse scatter operations, with the classifier's weights copied over and kept frozen. Two properties then hold by construction. On discrete $\{0,1\}$ adjacency inputs, the dense and sparse forms compute the same function, so the wrapper introduces no modelling change over the pretrained classifier. On continuous adjacency inputs, only the dense form is defined, since sparse scatter presupposes integer edge indices. The dense form therefore exposes $\tilde{A}$ as a float tensor in the computation graph, which makes $\partial z_{\text{logits}} / \partial \tilde{A}$ well-defined and allows $\nabla_\phi \mathcal{L}_{\text{GNN}}$ to flow back to the generator's parameters $\phi$.

Concretely, the sparse call \texttt{scatter\_add} on an edge list is replaced by a matrix product $\tilde{A}\,\mathbf{H}^{(t-1)}$ via \texttt{torch.matmul}. Autograd differentiates matrix multiplication directly: if $\mathbf{Y} = \tilde{A}\,\mathbf{H}$, then $\partial \mathbf{Y} / \partial \tilde{A}_{ij} = \mathbf{H}[j, :]$, and every entry of $\tilde{A}$ has a well-defined derivative. The chain rule no longer contains a zero factor.
