...
\paragraph{Atom-type distribution shift.} Figure~\ref{fig:mutag-node-type-shifts} shows the same composition result as generated-minus-real percentage-point shifts. This makes the overcompensation pattern visible: on class~0 the 1-2-GNN strongly increases halogen atoms, especially F and Br, while almost removing N; on class~1 the 1-GNN introduces halogens that are nearly absent in the real class, whereas the 1-2-GNN keeps halogens near zero and instead increases C.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/mutag/comparison/figures/fingerprint_node_type_shifts.png}
\caption{MUTAG atom-type distribution shifts. Bars show generated-minus-real changes in active-node atom shares, in percentage points, over 1000 generated graphs per class. Positive bars are over-produced by the generator; negative bars are under-produced.}
\label{fig:mutag-node-type-shifts}
\end{figure}
...
\paragraph{Edge-type co-occurrence fingerprint.} Cycles describe local topology; edge-type co-occurrence describes which atom pairs the generator places at the two ends of each edge. Figure~\ref{fig:mutag-edge-pairs} shows the largest generator-minus-real shifts for the 1-2-GNN generator, in percentage points. The class-1 panel makes the generator's bias visible: it concentrates bonds on common C/O/N backbone pairs and drops rare atom-pair types.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/mutag/comparison/figures/fingerprint_edge_pairs_12gnn.png}
\caption{MUTAG edge-type co-occurrence fingerprint for the 1-2-GNN generator. Bars show the largest generated-minus-real changes in unordered atom-pair edge shares, in percentage points, over 1000 generated graphs per class. Positive bars are over-produced by the generator; negative bars are under-produced.}
...
\paragraph{Secondary-structure distribution shift.} Figure~\ref{fig:proteins-node-type-shifts} plots generated-minus-real node-type shifts for PROTEINS. Class~0 is close to the real distribution for the 1-2-GNN and only mildly shifted for the 1-GNN. Class~1 is the main failure mode: both generators over-produce Sheet nodes and under-produce Helix and Coil/Turn, with the 1-2-GNN showing the strongest Sheet-heavy collapse.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/proteins/comparison/figures/fingerprint_node_type_shifts.png}
\caption{PROTEINS secondary-structure distribution shifts. Bars show generated-minus-real changes in active-node type shares, in percentage points, over 1000 generated graphs per class. Here H denotes Helix, S denotes Sheet, and C denotes Coil/Turn.}
\label{fig:proteins-node-type-shifts}
\end{figure}
...
\paragraph{Edge-type co-occurrence fingerprint.} Figure~\ref{fig:proteins-edge-pairs} shows the largest secondary-structure pair shifts on edges for the 1-2-GNN generator. The class-1 panel shows the main effect directly: Sheet-Sheet edges are over-produced, while Helix-Helix and Helix-Sheet edges are under-produced relative to real PROTEINS. This matches the table's 88.7\% Sheet share and shows that the generator collapses toward a Sheet-heavy prototype.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/proteins/comparison/figures/fingerprint_edge_pairs_12gnn.png}
\caption{PROTEINS edge-type co-occurrence fingerprint for the 1-2-GNN generator. Bars show the largest generated-minus-real changes in unordered secondary-structure edge-pair shares, in percentage points, over 1000 generated graphs per class. Positive bars are over-produced by the generator; negative bars are under-produced.}
