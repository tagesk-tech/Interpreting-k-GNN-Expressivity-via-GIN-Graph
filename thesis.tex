...
The edge-pair plot confirms this. In PROTEINS class 1, the 1-2-GNN strongly overproduces S-S edges and underproduces H-H and H-S edges. This matters because the generated graphs are not only changing the number of Sheet nodes. They are also changing how those nodes are connected.
...
The node-type and edge-pair figures show which parts of the graphs drive these prototypes. On MUTAG, the 1-2-GNN is strongly halogen-heavy for Non-Mutagen, but not for Mutagen. For Mutagen, it instead removes the rare halogens that appear more often in the 1-GNN generations. The edge-pair plots show that this is not only a node-count effect: the generated bonds also move toward different atom-pair patterns.

On PROTEINS, the clearest visual pattern is the Sheet-heavy shift in class~1. Both generators over-produce Sheet nodes, but the 1-2-GNN does it more strongly, and the edge-pair figure shows the same effect as an over-production of S-S edges. For class~0, the 1-2-GNN stays closer to the real node-type distribution and is accepted by both classifiers, while the 1-GNN Enzyme graphs are mostly rejected by the 1-2-GNN.

The cycle figures add the topological part of the analysis. On MUTAG, the 1-2-GNN produces fewer triangles than the 1-GNN, which fits the expected advantage of pair-level message passing, but it also creates too many 4-cycles. On PROTEINS, the generated graphs over-produce longer cycles even when mean degree is close to the real data. This suggests that the issue is not just the number of edges, but how the edges are arranged.
