...
Message passing on 2-sets uses a local neighbourhood in which each neighbour of a pair is obtained by replacing exactly one of its elements with a node adjacent (in the original graph) to the removed one. For a 2-set $s = \{u, v\}$, this neighbourhood decomposes into two cases:
\begin{equation}
