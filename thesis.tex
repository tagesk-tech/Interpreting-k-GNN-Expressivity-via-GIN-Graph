...
Each configuration generates 100 graphs and reports: (i) the validity rate, the fraction passing both the $v > 0.5$ and the 3$\sigma$ degree conditions; (ii) the mean validation score, averaged across all 100 graphs; (iii) the component decomposition $s$, $p$, $d$ (Table~\ref{tab:sanity}), where patterns such as high $p$ with low $s$ on PROTEINS Enzyme would not appear under the $s = p$ proxy; and (iv) the top-ranked explanations sorted by $v$, shown in the figures. All four quantities are forward-pass computations on frozen networks. No training or gradients are involved.
...
Figure~\ref{fig:training} shows a representative training curve for one of the four generators (MUTAG, 1-2-GNN, class 1). The three losses follow the expected pattern: during the first $\sim$120 epochs ($\lambda \approx 0$), the WGAN-GP losses dominate while the GNN guidance loss is inactive; as the dynamic weighting $\lambda(t)$ rises (Equation~\ref{eq:lambda}), the GNN loss decreases sharply and the adversarial losses plateau. All four generators reach the 300-epoch horizon without divergence on both datasets.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{results/mutag/12gnn_class1/figures/training.png}
\caption{Training curves for one GIN-Graph run (MUTAG, 1-2-GNN, class 1). Generator loss, discriminator loss, and GNN guidance loss over 300 epochs. The dynamic weighting $\lambda(t)$ shifts emphasis from adversarial realism (early) to class-specific guidance (late).}
\label{fig:training}
\end{figure}
...
\midrule
MUTAG    & 0 (Mutagen)    & 1-GNN   & 0.320 & 0.700 & 1.000 & 0.210 \\
MUTAG    & 0 (Mutagen)    & 1-2-GNN & 0.434 & 0.771 & 1.000 & 0.314 \\
MUTAG    & 1 (Non-Mut.)   & 1-GNN   & 0.681 & 0.935 & 1.000 & 0.490 \\
MUTAG    & 1 (Non-Mut.)   & 1-2-GNN & 0.735 & 0.933 & 1.000 & 0.565 \\
\midrule
PROTEINS & 0 (Non-Enz.)   & 1-GNN   & 0.986 & 0.987 & 0.986 & 0.984 \\
PROTEINS & 0 (Non-Enz.)   & 1-2-GNN & 0.760 & 0.782 & 0.564 & 0.996 \\
PROTEINS & 1 (Enzyme)     & 1-GNN   & 0.330 & 0.356 & 0.919 & 0.837 \\
PROTEINS & 1 (Enzyme)     & 1-2-GNN & 0.524 & 0.151 & 0.998 & 0.961 \\
\bottomrule
\end{tabular}
\end{table}
...
\end{subfigure}
\caption{Top-ranked GIN-Graph explanations on MUTAG class 0 (Mutagen). Node colours indicate atom types (see Figure~\ref{fig:legend}).}
...
Two cells stand out. The 1-2-GNN's Non-Mutagen graphs on MUTAG are accepted as Non-Mutagen by the 1-2-GNN (100\%) but classified as Mutagen by the 1-GNN (cross 12.4\%). Symmetrically, the 1-GNN's Non-Enzyme graphs on PROTEINS are accepted by the 1-GNN (100\%) but classified as Enzyme by the 1-2-GNN (cross 9.4\%). Each classifier has a class whose defining features, as captured by that classifier's generator, are not visible to the other classifier.
