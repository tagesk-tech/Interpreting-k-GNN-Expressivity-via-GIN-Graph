...
All $k$-GNN models use a hidden dimension of $d_T = 64$. The 1-GNN uses $T_1 = 3$ message-passing layers. The 1-2-GNN uses 3 layers for the 1-GNN component and $T_2 = 2$ layers for the 2-GNN component. Both use dropout of 0.5 and are trained with Adam (lr = 0.01) using cross-entropy loss. Table~\ref{tab:kgnn-results} reports the best epoch observed for each run. Data is split 80/20 into train/test with a fixed seed of 42.
