...
The generated graphs are not random samples from the real class distribution. GIN-Graph optimises for graphs the frozen classifier accepts as the target class. The discriminator and the three structural-regularisation losses push back during training, and a degree-based validity gate filters the output at evaluation time. So the right reading of the populations is ``what does this classifier find convincing as a class member?'', not ``what does a real MUTAG molecule look like?''

The clearest evidence is the cross-model classification table. Most cells transfer: a graph that one model accepts is also accepted by the other. Two cells don't. The 1-2-GNN's MUTAG Mutagen graphs are accepted by the 1-2-GNN but rejected by the 1-GNN at 12.3\% agreement, and the 1-GNN's PROTEINS Enzyme graphs are accepted by the 1-GNN but rejected by the 1-2-GNN at 14.0\%. The disagreement is class-specific, not architectural.

The asymmetry is consistent with the architectural difference. The 1-GNN's representations are bounded by what 1-WL can encode: node labels and their neighbourhood structure, propagated by repeated aggregation. The 1-2-GNN adds a second representation per node pair, including the edge indicator between the two endpoints, which makes short-cycle structure visible to the classifier. The generator still only outputs nodes and edges, but the 1-2-GNN reads those outputs through a richer pair-level representation, so the prototype can move along axes the 1-GNN can't see.

The MUTAG fingerprints make this concrete. For Non-Mutagen, the 1-2-GNN's generated population is heavily over-halogenated: $+26$\,pp F, $+11$\,pp Br, with the matching C-F and C-Br edges. For Mutagen, the same architecture goes the other way and almost zeros halogens out, raising carbon and shifting the edge mix toward C-O and O-O. So this isn't a property of the 1-2-GNN architecture as such; it learns class-specific prototypes that disagree with the 1-GNN's in different directions.

The cycle counts are where the theory motivation actually shows up. Real MUTAG molecules in this split have no 3- or 4-cycles, and both generators introduce them, so neither population is a faithful sample of MUTAG. Even so, the 1-2-GNN produces fewer triangles than the 1-GNN in both classes, and far fewer in Mutagen (0.15 vs 1.01 per graph). That's the one signal in the thesis that lines up with what 2-WL can do and 1-WL can't. We don't generalise the claim. The 1-2-GNN over-produces 4-cycles in the same Mutagen prototype, so the 2-WL distinction doesn't make the explanations uniformly more realistic; it changes which structures the classifier finds useful.

The overshooting in some populations is consistent with how the loss is set up. The generator only has to satisfy the classifier, and once it finds a feature the classifier likes, the GNN-guidance term will keep pushing on that feature unless something else holds it back. The Sheet share on PROTEINS Non-Enzyme (88.7\% vs 56.2\% real) and the halogen share on MUTAG Non-Mutagen (41.7\% vs 4.1\%) both look like that kind of amplification. Matching average degree doesn't constrain the rest of the structure, so the generator is free to push these features past their real frequencies.

PROTEINS shows the limit of degree-only structural validation. Most generated PROTEINS graphs match the real mean degree, but they don't match the real topology. Many real proteins look chain-like in Figure~\ref{fig:proteins-examples}; the generated populations look like dense compact blobs. The setup partly invites this: generation is capped at $N=50$, connectedness isn't enforced, and the validity gate only checks average degree. The generator ends up arranging a realistic number of edges into an unrealistic number of closures, and the normalized cycle plots show the excess remains after dividing out graph size and edge count.

This matters most for PROTEINS Enzyme. The two generators end up with similar Helix/Sheet proportions, and the 1-2-GNN's are closer to real than the 1-GNN's. But the 1-2-GNN still rejects 86\% of the 1-GNN's Enzyme graphs. So the rejection isn't about node composition. The 1-2-GNN is reading something else, presumably the connectivity pattern between Helix and Sheet nodes that the 1-GNN can't shape. Non-Enzyme is a different story: both generators go Sheet-heavy, but the 1-2-GNN goes further and also shifts the edge mix toward S-S, so the prototype diverges in both composition and topology.

The picture, then, is mixed. The 1-2-GNN learns different class prototypes than the 1-GNN even at the same accuracy, and on MUTAG triangle suppression those different prototypes line up with what theory predicts. The rest of the shifts (halogen ratios, Sheet excess, long-cycle drift) are model-specific effects we can describe but not derive from the WL hierarchy alone. The generated graphs are useful because they make these differences visible, but the comparison doesn't show that one architecture's class concept is uniformly closer to reality.
