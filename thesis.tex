...
At the same time, the 1-2-GNN overproduces 4-cycles, especially in class 1. This means the 1-2-GNN is not simply more realistic. It seems to avoid one unrealistic structure, triangles, while creating another kind of closure. A careful interpretation is that the 1-2-GNN learns a different structural prototype, not a universally better one. The strongest MUTAG result is still the cross-model result for class 1: 1-2-GNN generated Mutagen graphs are accepted by the 1-2-GNN, but mostly rejected by the 1-GNN.
...
The cross-classification result for PROTEINS class 0 is stronger than the composition table alone. The 1-GNN generated Enzyme graphs are accepted by the 1-GNN, but mostly rejected by the 1-2-GNN. The reverse is not true in the same way: 1-2-GNN generated Enzyme graphs are accepted by both models. This suggests that the 1-2-GNN decision boundary for Enzyme is more selective in this experiment.
...
\caption{PROTEINS simple-cycle counts per graph, aggregated over 1000 generated graphs per cell. Error bars are the standard error of the mean. Both generators over-produce longer cycles relative to real; the strongest differences appear in the 6-cycle panels, with the 1-2-GNN highest in class~0 and the 1-GNN highest in class~1.}
\label{fig:proteins-cycles}
\end{figure}

\subsection{Summary of the comparison}

Across both datasets, the generated graphs should be interpreted as classifier-specific prototypes rather than faithful random samples from the real data. The generators usually produce graphs that their own classifier accepts with high confidence, but the generated populations often differ from the real data in systematic ways. These differences appear in graph size, node-type composition, edge-pair composition, and cycle structure.

The main finding is that the 1-GNN and 1-2-GNN do not learn the same class prototypes. On MUTAG, the clearest difference appears for class 1, where 1-2-GNN generated Mutagen graphs are mostly rejected by the 1-GNN. On PROTEINS, the clearest difference appears for class 0, where 1-GNN generated Enzyme graphs are mostly rejected by the 1-2-GNN. These cross-model failures show that the generated graphs are not just noisy versions of the same underlying class idea. Each architecture can learn class features that the other architecture does not treat as sufficient.

The results therefore support a cautious conclusion. Higher-order message passing changes what the model treats as a convincing class example, but it does not make the generated explanations uniformly more realistic. In some cases, the 1-2-GNN looks closer to the real distribution. In other cases, it overcompensates more strongly. The value of the qualitative analysis is that it makes these differences visible.
...
\subsection{What the qualitative figures add}

The figures in Section~\ref{sec:qualitative} add a more concrete explanation of where the two models differ. The cross-model classification figures show that most generated graphs are accepted by the model that produced them, but not always by the other model. This is important because it means the generated graphs are not just generic class samples. They are model-specific prototypes.

The node-type and edge-pair figures show which parts of the graphs drive these prototypes. On MUTAG, the 1-2-GNN is strongly halogen-heavy for Non-Mutagen, but not for Mutagen. For Mutagen, it instead removes the rare halogens that appear more often in the 1-GNN generations, increases carbon, and lowers nitrogen. The edge-pair plots show that this is not only a node-count effect: the generated bonds also move toward different atom-pair patterns, such as more C-O and O-O in the 1-2-GNN Mutagen case.

On PROTEINS, the clearest visual pattern is the Sheet-heavy shift in class~1. Both generators over-produce Sheet nodes, but the 1-2-GNN does it more strongly, and the edge-pair figure shows the same effect as an over-production of S-S edges. For class~0, the 1-2-GNN stays closer to the real node-type distribution and is accepted by both classifiers, while the 1-GNN Enzyme graphs are mostly rejected by the 1-2-GNN. The edge-pair plot also shows that class~0 is not perfectly unchanged: the 1-2-GNN shifts some connectivity from S-S toward H-S.

The cycle figures add the topological part of the analysis. On MUTAG, the 1-2-GNN produces fewer triangles than the 1-GNN, which fits the expected advantage of pair-level message passing, but it also creates too many 4-cycles. On PROTEINS, the generated graphs over-produce longer cycles even when mean degree is close to the real data. The pattern is class-specific: the 1-2-GNN has the strongest class~0 6-cycle excess, while the 1-GNN is more cycle-heavy on class~1. This suggests that the issue is not just the number of edges, but how the edges are arranged.

Taken together, the figures support the main interpretation of the thesis: higher-order message passing changes the learned class prototype, but it does not automatically make the explanation more realistic in every class.
