...
GIN-Graph isn't trained to draw random samples from the real class. It's trained to find graphs the classifier accepts as the target class. The discriminator and three regularisers (size, type, degree) keep the search inside something realistic-looking, and at evaluation a degree-based gate filters the rest. So we should read the generated populations as the answer to ``what convinces this classifier?'', not as a substitute for the real MUTAG distribution.
