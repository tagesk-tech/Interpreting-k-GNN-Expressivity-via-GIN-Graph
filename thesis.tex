...
\end{subfigure}
\caption{Top-ranked GIN-Graph explanations on MUTAG class 0 (Non-Mutagen). The 1-GNN repeatedly produces mixed C/N/O scaffolds with scattered halogens that stay near the real non-mutagen composition, whereas the 1-2-GNN collapses onto narrower fluorine- and bromine-heavy prototype families that drift away from it. Image titles show the legacy class labels from earlier figure generation and will be regenerated after the handler fix; the class-0 index and underlying graphs are unchanged. Node colours indicate atom types (see Figure~\ref{fig:legend}).}
...
\end{subfigure}
\caption{Top-ranked GIN-Graph explanations on PROTEINS (1-GNN). The Enzyme explanations (class 0) are large balanced H/S structures near the 50-node cap, while the Non-Enzyme explanations (class 1) are more variable in size and shape. Image titles show the legacy class labels from earlier figure generation and will be regenerated after the handler fix. Node colours indicate secondary structure types: helix (H, pink), sheet (S, teal), coil/turn (C, green).}
...
\end{subfigure}
\caption{Top-ranked GIN-Graph explanations on PROTEINS (1-2-GNN). The Enzyme explanations (class 0) are highly regular 50-node H/S structures, whereas the Non-Enzyme explanations (class 1) repeatedly collapse onto sheet-dominated cores with smaller helix appendages. Class 0 explanations achieve validation scores $v = 0.76$--$0.82$ with 50 nodes, while class 1 explanations achieve $v = 0.52$--$0.58$ with 29--32 nodes. Image titles show the legacy class labels from earlier figure generation.}
...
\caption{Graph-level normalized node-type entropy for the real training subsets and the generated datasets. Lower entropy indicates more homogeneous node-type composition within each graph. The strongest drop appears for PROTEINS Non-Enzyme (class 1) under the 1-2-GNN, while the other classes show different shifts rather than one uniform trend. Figure titles reflect the legacy inverted MUTAG/PROTEINS class names from the handler at the time of generation; the correct mapping is class 0 = Non-Mutagen / Enzyme and class 1 = Mutagen / Non-Enzyme, and the underlying numbers are unchanged.}
