...
All $k$-GNN models use a hidden dimension of $d_T = 64$. The 1-GNN uses $T_1 = 3$ message-passing layers; the 1-2-GNN uses 3 layers for the 1-GNN component and $T_2 = 2$ layers for the 2-GNN component. Both use dropout of 0.5 and are trained with Adam (lr = 0.01) and cross-entropy loss. The reported classifier checkpoints come from 300-epoch runs on a fixed 80/20 train/test split with seed 42.
...
\[
  H_{\mathrm{norm}}(G) = \frac{-\sum_t p_t(G)\log p_t(G)}{\log K},
\]
where $p_t(G)$ is the proportion of node type $t$ in graph $G$ and $K$ is the number of node types in the dataset. Lower values mean that the graphs are more compositionally homogeneous, while higher values mean that the node types are more mixed. Figure~\ref{fig:node-entropy} shows that these entropy shifts are again class-dependent. The clearest low-entropy case is PROTEINS Non-Enzyme (class 1), where the 1-2-GNN generated graphs have much lower median entropy than both the real graphs (0.303 vs.\ 0.536) and the 1-GNN graphs (0.303 vs.\ 0.525), which matches the strongly sheet-dominated explanation family. On MUTAG Mutagen (class 1) the same pattern appears more moderately (median 0.301 for 1-2-GNN vs.\ 0.545 for 1-GNN). On MUTAG Non-Mutagen (class 0), the more notable result is that both generators are more heterogeneous than the real class (medians 0.621 and 0.677 vs.\ 0.463), not just that the 1-2-GNN sits above the 1-GNN. On PROTEINS Enzyme (class 0), both generators collapse to an almost fixed compositional template with near-zero variance in entropy (standard deviations 0.003 for 1-GNN and 0.005 for 1-2-GNN, versus 0.143 for the real graphs). So the higher-order model does not simply make the generated graphs more homogeneous in every case. Instead, both architectures can collapse toward very regular prototype families, but they do so in different ways depending on the class.

Reading across the four panels alongside the population composition in Table~\ref{tab:node-types}, a consistent pattern appears: generator entropy tracks whichever node type the generator has settled on, rather than the class's natural heterogeneity. On PROTEINS Enzyme the near-zero entropy variance means that the generators are essentially emitting a single compositional template (H/S ratio fixed within 0.3 percentage points across 500 graphs), which directly supports Claim~3 (Section~\ref{sec:claim3}). The outputs are prototypes, not draws from the real class distribution. The PROTEINS Non-Enzyme collapse under the 1-2-GNN is both compositional (89\% S) and structural (size clusters at 29--32 nodes), so the visible entropy drop and the sheet-heaviness in Figure~\ref{fig:proteins-12gnn-explanations} are two views of the same prototype-family collapse. On MUTAG Non-Mutagen the direction flips: \emph{both} generators are \emph{more} heterogeneous than the real class, driven by halogens and fluorine that the real non-mutagen distribution contains only at single-digit percentages. This is consistent with GIN-Graph's generator producing a model-pleasing ``non-mutagen-like mixture'' rather than sampling the real composition, again supporting Claim~3.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{results/summary/normalized_node_entropy.png}
\caption{Graph-level normalized node-type entropy for the full real dataset split by class (n = 63, 125, 663, 450 for MUTAG Non-Mutagen, MUTAG Mutagen, PROTEINS Enzyme, PROTEINS Non-Enzyme) and the 500-graph generated datasets. Lower entropy indicates more homogeneous node-type composition within each graph. The strongest drop appears for PROTEINS Non-Enzyme (class 1) under the 1-2-GNN (median 0.303 vs.\ 0.536 real), and PROTEINS Enzyme (class 0) shows near-zero variance for both generators ($\sigma \leq 0.005$ vs.\ real $\sigma = 0.143$), indicating collapse onto a single compositional template.}
\label{fig:node-entropy}
\end{figure}
