...
% ============================================================
% Title page (KTH KEX template, adapted for pdflatex with Helvetica)
% ============================================================
\makeatletter
\newcommand{\@authorone}{}
\newcommand{\@authortwo}{}
...
\renewcommand\maketitle{%
  ...
  \vspace*{0.50cm}
  \begin{center}
    \hspace{0.1cm}\includegraphics[width=3.06cm]{misc/image2.png}\\
    \vspace*{0.69cm}
    \setstretch{0.0}
    \setlength{\parskip}{0pt}
    \vspace{60pt}
    {\fontsize{12pt}{0pt}\selectfont Examensarbete inom teknik}\\
    \vspace{12.96pt}
    {\fontsize{10pt}{0pt}\selectfont Grundnivå, 15 hp}\\
    \vspace{45pt}
    {\bfseries\fontsize{26pt}{30pt}\selectfont \@title}\\
    \vspace{18pt}
    {\fontsize{16pt}{20pt}\selectfont \@subtitle}\\
    \vspace{33pt}
    {\bfseries\fontsize{12pt}{0pt}\selectfont \@authorone}\\
    \vspace{31pt}
    {\bfseries\fontsize{12pt}{0pt}\selectfont \@authortwo}\\
  \end{center}
  \restoregeometry
  \endgroup
}
...
\title{Hierarchical $k$-GNN Explanations via Generative Interpretation Networks}
\subtitle{Comparing 1-GNN and 1-2-GNN for Model-Level Graph Classification Interpretability}
\authorone{Tage Sköldefors}
\authortwo{Maximillian Nordler}
\date{March 2026}
...
\begin{equation}
  \mathbf{f}_s^{(0)} = \left[\mathbf{h}_u^{(T)} \;\|\; \mathbf{h}_v^{(T)} \;\|\; A_{uv}\right] \in \mathbb{R}^{2d_T + 1},
  \label{eq:2set-feat}
\end{equation}
...
The qualitative results should be read as generated tests of classifier behaviour, not as samples from the true data distribution. GIN-Graph searches for graphs that a frozen classifier accepts as evidence for a target class. During training, the search is constrained by the discriminator, size regularisation, and type regularisation; at evaluation time, a degree-based validity gate filters the outputs. The generated populations therefore answer a narrower question: what kind of graph can be made convincing to this classifier? They do not show what every real MUTAG molecule or PROTEINS graph should look like.
...
A plausible reason is that the 1-2-GNN exposes different structure to the classifier. The 1-GNN aggregates node neighbourhoods across repeated message-passing layers, so its representations are limited to the structural distinctions that 1-WL-style updates can encode. The 1-2-GNN additionally builds representations over node pairs, with pair features containing both endpoint embeddings and the edge indicator between them. Its pair-level neighbourhoods make edge-pair patterns, closures, and short-cycle structure more directly available to the model. The generator still only outputs nodes and edges, but those outputs are interpreted through a richer pair-level representation. This can make the generated prototype move in directions that are weak or invisible for the 1-GNN.

On MUTAG, the population fingerprints show that the difference is not a single-feature effect. For Non-Mutagen, the 1-2-GNN generator produces a much more halogen-heavy population than the real data, especially through F and Br atoms and corresponding C-F and C-Br edges. For Mutagen, the same architecture moves halogens in the opposite direction, suppressing rather than enriching them, while increasing carbon and shifting edge-pair composition toward C-O and O-O edges. This makes the MUTAG result class-dependent rather than a simple statement that the 1-2-GNN prefers more halogens.
